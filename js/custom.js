...
function calculateSquarePrice() {
    ...
    // count
    let countToAdd = $('#countToAdd').val();

    if ($('#length').length) {
        // has calculator
        let length = $('#length').val() / 1000;

        // square
        let square = length * width;
        square = square * countToAdd;
        $('#totalSquare').text(square.toFixed(2));
        $('#totalSquareInput').val(square.toFixed(2));

        // price
        let calculatedPrice = price * square;
        $('#totalPrice').text(calculatedPrice.toFixed(2));
        $('#totalPriceInput').val(calculatedPrice.toFixed(2));
    } else {
        // price
        let calculatedPrice = price * countToAdd;
        $('#totalPrice').text(calculatedPrice.toFixed(2));
        $('#totalPriceInput').val(calculatedPrice.toFixed(2));
    }
