<div class="swiper-slide itemsSlider__slide slider_item" id="{{ $sliderProduct->categories->first()->title }}">
    <div class="card newItems__card" data-product="{{ $sliderProduct->id }}">
        <a class="card__imgBox" href="{{ route('index.products.show',
            ['product' => $sliderProduct->slug, 'category' => $sliderProduct->categories->first()->slug]) }}">
            <picture>
                <source type="image/webp" srcset="{{ $sliderProduct->mainPhotoPath() }}">
                <img src="{{ $sliderProduct->mainPhotoPath() }}" alt="ni1">
            </picture>
        </a>
...
{{--        </ul>--}}
        ...
        <div class="card__controllers">
            ...
            <div class="card__icons">
                <div class="card__icon card__icon--like addTo {{ product_id_in_list($sliderProduct->id, 'favorites') ? 'active' : '' }}" data-destination="Favorites" role="button" tabindex="0">
                    <svg>
                        <use xlink:href="{{ asset('img/sprites/sprite-mono.svg#heart') }}"></use>
                    </svg>
                </div>
                <div class="card__icon card__icon--stat addTo {{ product_id_in_list($sliderProduct->id, 'compare') ? 'active' : '' }}" data-destination="Compare" role="button" tabindex="0">
                    <svg>
