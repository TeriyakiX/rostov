...
(function ($) {
    ...
    function minItemsAlert() {
        ...
        if ($alert.length === 0) {
            ...
            $alert = $('<div class="dropdown-minItem-alert"></div>');
        }
    ...
    function selectToDiv(str) {
        ...
        result = result.replace(/<optgroup[^>]*>/gi, function (matcher) {
            ...
            parentArray.push(groupId[1])
            return '<li class="dropdown-group checkAllProduct" style="cursor: pointer" data-group-id="' + (groupId ? groupId[1] : '') + '">' + (groupName ? groupName[1] : '') + '</li>';
        });

        result = result.replace(/<option(.*?)<\/option>/gi, function (matcher) {
            // let grId = parentArray[a];
            ...
            return '<li ' + (isDisabled ? ' disabled' : ' tabindex="0"') + ' data-value="' + (value || '') + '" class="dropdown-option jjj '+  a + 'jjj ' + (isSelected ? 'dropdown-chose' : '') + '" ' + extendAttr + '>' + (name ? name[1] : '') + '</li>';
        });
        return result;
    }
    ...
    var action = {
        ...
        categoryCheck(){
            if(!$(this).hasClass('clicked_cat')){
                $(this).addClass('clicked_cat')
            }else {
                $(this).removeClass('clicked_cat')
            }

            let ids = []
            let csrf = $('meta[name=csrf_token]').attr('content');
            $.ajax({
                url: '/admin/entity/check-all-products',
                data: {_token: csrf, categoryName : $(this).html()},
                type: "POST",
                async: false,
            ...
            let li = $(this).parent().children('li[tabindex]');
            if(ids.length > 0){
                if ($(this).hasClass('clicked_cat')) {
                    $.each(li, (k, el) => {
                        if (ids.includes(parseInt($(el).attr('data-value')))) {
                            $(el).addClass('dropdown-chose');
                            $(el).trigger('click');
                        }
                    })
                }else {
    ...
    Dropdown.prototype = {
        ...
        bindEvent: function () {
            ...
            $el.on(EVENT_SPACE.click, '.checkAllProduct', $.proxy(action.categoryCheck));
    ...
    $(document).on('click', '.dropdown-search', function () {
        alert(555);
    });
